...
#[test]
fn upgrade_reports_missing_template_files() {
    let project = TempProject::with_git();
    init_project(&project);

    // Delete a file that was generated by init and has a tracked hash
    let config_content = project.read_file(".agents/harn/config.toml");
    let config: toml::Table = toml::from_str(&config_content).unwrap();
    let file_hashes = config["init"]["file_hashes"]
        .as_table()
        .expect("file_hashes should be a table");

    if let Some(first_file) = file_hashes.keys().next() {
        let path = project.path().join(first_file);
        if path.exists() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(
            stdout.contains("missing") || stdout.contains("new") || stdout.contains("created"),
            "upgrade should handle missing template files, got: {stdout}"
        );
    }
}
